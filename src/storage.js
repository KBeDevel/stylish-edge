...
function sectionAppliesToUrl(section, url) {
	// only http, https, file, and ms-browser-extension allowed
	if (url.indexOf("http") != 0 && url.indexOf("file") != 0 && url.indexOf("ms-browser-extension") != 0 && url.indexOf("ftp") != 0) {
		return false;
	}
	// other extensions can't be styled
	if (url.indexOf("ms-browser-extension") == 0 && url.indexOf(chrome.extension.getURL("")) != 0) {
		return false;
    }
